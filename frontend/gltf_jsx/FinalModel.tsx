...
export default function FinalModel({ ...props }: MageType) {
  ...
  useFrame((state, delta) => {
    ...
    if (tl.current && postl.current) {
      // console.log(scroll.offset);
      tl.current.seek(scroll.offset * 2.8 * tl.current.duration() * 0.7)
      // console.log(scroll.offset * postl.current.duration() * 0.7);
      // console.log(scroll.offset * postl.current.duration() * 30);

      postl.current.seek(scroll.offset * 2.8 * postl.current.duration() * 30)
    }
    if (!scroll.offset) {
      store.activeAnim = "BALL"
      store.prevAnim = "JUMP"
    }
    if (actions && scroll.offset > 0 && scroll.offset * 2.8 <= 0.08) {
      store.prevAnim = "BALL"
      // console.log(scroll.offset * 2.8);
      // console.log(actions[activeAnim]!.getClip().duration );
      store.activeAnim = "JUMP"
      actions[activeAnim]!.time =
        actions[activeAnim]!.getClip().duration * scroll.offset * 2.8 * 5
    }
    // if (scroll.offset * 2.8 > 0.03 && scroll.offset * 2.8 <= 0.07) {
    //   actions[activeAnim]!.time =
    //     actions[activeAnim]!.getClip().duration * scroll.offset * 2.8 * 5;
    // }
    if (scroll.offset * 2.8 > 0.08 && scroll.offset * 2.8 <= 1) {
      // console.log("flying");
      store.prevAnim = "JUMP"
